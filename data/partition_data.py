...
def create_person_partitions(
    ...
    for idx, person in enumerate(person_names):
        if not split_modalities:
            ...
            for naming_idx, naming in namings:
                multimodal_data = import_multiple_modalities(
                    data_type=data_type,
                    data_directory=data_directory,
                    namings=namings,
                ...
                partition_idx = idx * len(namings) + naming_idx
                save_partition(
                    partition=train_parition,
                    idx=partition_idx,
                    fed_dir=fed_dir,
...
def get_client_local_model(path_to_data: Path, cid: str, model_type: str, ext="pt"):
    client_dir = path_to_data / f"{cid}"
    model = torch.load(client_dir / f"{model_type}.{ext}")
    return model
