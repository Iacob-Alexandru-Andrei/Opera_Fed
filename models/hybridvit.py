...
class CNNEncoder(nn.Module):
    def __init__(self,
                 ...
                 pooling_kernel_size = 7, pooling_stride = 1, pooling_padding = 1,
                 activation= nn.ReLU(),
...
class HybridViT(nn.Module):
    def __init__(self, *, image_size, patch_size, 
                 ...
                 seq_pool = False, positional_embedding = True,
                 emb_dropout = 0.):
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange('b c (h p1) (w p2) -> b (h w) (p1 c) (p2 )', p1 = patch_height, p2 = patch_width),
        )
        ...
        if positional_embedding:
            self.positional_emb = nn.Parameter(torch.randn(1, num_patches + 1, dim), requires_grad = True)
    ...
    def forward(self, img):
        ...
        x = torch.cat(tuple(self.encoder(x[:, i:i+1]).unsqueeze(dim = 1) for i in range(x.shape[1])), axis = 1)
